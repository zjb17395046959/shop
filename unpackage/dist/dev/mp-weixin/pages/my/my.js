...
/* harmony import */ var _my_vue_vue_type_style_index_0_lang_scss___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./my.vue?vue&type=style&index=0&lang=scss& */ 62);
...
//
//
var _default = { data: function data() {return { is_token: false, ipone: '', //导航列表数据
      ...
      my_list: [{ id: 1, title: '钱包', icon: 'qianbao', img: '/static/img/right.png' }, { id: 2, title: '收藏', icon: 'shoucang', img: '/static/img/right.png' }, { id: 3, title: '优惠券', icon: '9', img: '/static/img/right.png' }, { id: 4, title: '收货地址', icon: 'shouhuodizhi', img: '/static/img/right.png' }], img_url: 'https://vkceyugu.cdn.bspapp.com/VKCEYUGU-aliyun-wm60vejtdast68da9b/bf83d600-1ffc-11eb-880a-0db19f4f74bb.jpg', userName: '' };}, onShow: function onShow() {//每次进入页面重新获取数据，这样的话，可以使用页面刷新，就可以使数据达到实时更新，
    // this.token = uni.getStorageSync('token')||'';
    this.img_url = uni.getStorageSync('user').avatarUrl || 'https://vkceyugu.cdn.bspapp.com/VKCEYUGU-aliyun-wm60vejtdast68da9b/bf83d600-1ffc-11eb-880a-0db19f4f74bb.jpg';this.userName = uni.getStorageSync('user').nickName || '';this.ipone = uni.getStorageSync('ipone') || '';}, methods: { wx_user: function wx_user(e) {//点击获取用户信息
      var that = this;uni.showLoading({ title: '登录中...' });uni.login({ success: function success(res) {uniCloud.callFunction({ name: 'user-center', data: { action: 'loginByWeixin', params: { code: res.code } }, success: function success(ret) {if (ret.result.code == 0) {that.is_token = true;uni.hideLoading();that.img_url = e.detail.userInfo.avatarUrl;that.userName = e.detail.userInfo.userName;
